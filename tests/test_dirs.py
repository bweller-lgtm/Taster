"""Tests for taster.dirs — platform-aware config directory resolution."""
...
class TestGetConfigDir:
    def test_windows(self):
        with patch("taster.dirs.platform.system", return_value="Windows"):
            result = get_config_dir()
            assert result == Path.home() / "AppData" / "Roaming" / "taster"

    def test_macos(self):
        with patch("taster.dirs.platform.system", return_value="Darwin"):
            result = get_config_dir()
            assert result == Path.home() / "Library" / "Application Support" / "taster"
    ...
    def test_linux_default(self):
        ...
        with (
            patch("taster.dirs.platform.system", return_value="Linux"),
            patch.dict(os.environ, env, clear=True),
        ):
            result = get_config_dir()
            assert result == Path.home() / ".config" / "taster"
...
class TestEnsureDirs:
    def test_creates_config_dir(self, tmp_path):
        config_dir = tmp_path / "cfg" / "taster"
        with patch("taster.dirs.get_config_dir", return_value=config_dir):
            with patch("taster.dirs.get_profiles_dir", return_value=config_dir / "profiles"):
                result = ensure_dirs()
...
class TestFindProfilesDir:
    ...
    def test_falls_back_to_user_dir(self, tmp_path, monkeypatch):
        ...
        with patch("taster.dirs.get_profiles_dir", return_value=user_profiles):
            result = find_profiles_dir()
